<?php
/**
 * CBT System - Composite Exam Management
 * Full-featured management matching regular exams.php
 */

define('CBT_SYSTEM', true);
require_once __DIR__ . '/../includes/auth.php';

$pageTitle = 'Composite Exams';
$db = getDB();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reset_action'])) {
    ...
    if (!validateCSRFToken($csrf_token)) {
        redirect(APP_URL . '/admin/composite_exams.php', 'Invalid security token', 'error');
    }
    ...
    try {
        switch ($resetAction) {
            case 'reset_single_student':
                ...
                if ($studentId <= 0) {
                    redirect(APP_URL . '/admin/composite_exams.php', 'No student selected', 'error');
                }
                
                // Get details for logging
                $stmt = $db->prepare("SELECT s.matric_number, ce.exam_code 
                                      FROM students s, composite_exams ce 
                                      WHERE s.id = ? AND ce.id = ?");
                $stmt->execute([$studentId, $examId]);
                $details = $stmt->fetch();
                
                if (!$details) {
                    redirect(APP_URL . '/admin/composite_exams.php', 'Student or exam not found', 'error');
                }
                
                // Delete composite exam attempt and answers
                $db->prepare("DELETE FROM composite_exam_answers WHERE attempt_id IN (
                    SELECT id FROM composite_exam_attempts WHERE composite_exam_id = ? AND student_id = ?
                )")->execute([$examId, $studentId]);
                
                $db->prepare("DELETE FROM composite_exam_attempts WHERE composite_exam_id = ? AND student_id = ?")
                    ->execute([$examId, $studentId]);
                
                // Also delete any result rows
                $db->prepare("DELETE FROM composite_exam_results WHERE composite_exam_id = ? AND student_id = ?")
                    ->execute([$examId, $studentId]);
                ...
                logActivity('admin', $_SESSION['admin_id'], 'reset_composite_student_exam', 
                           "Reset composite exam for student: {$details['matric_number']} on exam: {$details['exam_code']}");
                redirect(APP_URL . '/admin/composite_exams.php', "Exam reset for student: {$details['matric_number']}");
                break;
            ...
            case 'reset_all_students':
                ...
                // Get exam details
                $stmt = $db->prepare("SELECT exam_code FROM composite_exams WHERE id = ?");
                $stmt->execute([$examId]);
                $exam = $stmt->fetch();
                
                if (!$exam) {
                    redirect(APP_URL . '/admin/composite_exams.php', 'Exam not found', 'error');
                }
                
                // Count students
                $countStmt = $db->prepare("SELECT COUNT(*) FROM composite_exam_students WHERE composite_exam_id = ?");
                $countStmt->execute([$examId]);
                $studentCount = $countStmt->fetchColumn();
                
                // Delete all answers, attempts, and results
                $db->prepare("DELETE FROM composite_exam_answers WHERE attempt_id IN (
                    SELECT id FROM composite_exam_attempts WHERE composite_exam_id = ?
                )")->execute([$examId]);
                
                $db->prepare("DELETE FROM composite_exam_attempts WHERE composite_exam_id = ?")->execute([$examId]);
                $db->prepare("DELETE FROM composite_exam_results WHERE composite_exam_id = ?")->execute([$examId]);
                
                // Reset assignments
                $db->prepare("UPDATE composite_exam_students SET status = 'assigned' WHERE composite_exam_id = ?")
                    ->execute([$examId]);
                
                logActivity('admin', $_SESSION['admin_id'], 'reset_all_composite_students_exam', 
                           "Reset composite exam for all $studentCount students on exam: {$exam['exam_code']}");
                redirect(APP_URL . '/admin/composite_exams.php', "$studentCount students have been reset for exam: {$exam['exam_code']}");
                break;
        }
    } catch (PDOException $e) {
        error_log("Reset composite exam error: " . $e->getMessage());
        redirect(APP_URL . '/admin/composite_exams.php', 'Error: ' . $e->getMessage(), 'error');
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['exam_id']) && !isset($_POST['reset_action'])) {
    ...
    try {
        switch ($action) {
            case 'publish':
                $db->prepare("UPDATE composite_exams SET status = 'published' WHERE id = ? AND status = 'draft'")->execute([$examId]);
                logActivity('admin', $_SESSION['admin_id'], 'publish_composite_exam', "Published composite exam ID: $examId");
                redirect(APP_URL . '/admin/composite_exams.php', 'Composite exam published successfully');
                break;
            case 'start':
                $db->prepare("UPDATE composite_exams SET status = 'running' WHERE id = ? AND status = 'published'")->execute([$examId]);
                logActivity('admin', $_SESSION['admin_id'], 'start_composite_exam', "Started composite exam ID: $examId");
                redirect(APP_URL . '/admin/composite_exams.php', 'Composite exam started successfully');
                break;
            case 'end':
                $db->prepare("UPDATE composite_exams SET status = 'ended' WHERE id = ? AND status = 'running'")->execute([$examId]);
                logActivity('admin', $_SESSION['admin_id'], 'end_composite_exam', "Ended composite exam ID: $examId");
                redirect(APP_URL . '/admin/composite_exams.php', 'Composite exam ended successfully');
                break;
            case 'archive':
                $db->prepare("UPDATE composite_exams SET status = 'archived' WHERE id = ? AND status = 'ended'")->execute([$examId]);
                logActivity('admin', $_SESSION['admin_id'], 'archive_composite_exam', "Archived composite exam ID: $examId");
                redirect(APP_URL . '/admin/composite_exams.php', 'Composite exam archived');
                break;
            case 'republish':
                $db->prepare("UPDATE composite_exams SET status = 'published' WHERE id = ? AND status = 'archived'")->execute([$examId]);
                logActivity('admin', $_SESSION['admin_id'], 'republish_composite_exam', "Republished composite exam ID: $examId");
                redirect(APP_URL . '/admin/composite_exams.php', 'Composite exam republished successfully');
                break;
            case 'delete':
                $db->prepare("DELETE FROM composite_exams WHERE id = ? AND status = 'draft'")->execute([$examId]);
                logActivity('admin', $_SESSION['admin_id'], 'delete_composite_exam', "Deleted composite exam ID: $examId");
                redirect(APP_URL . '/admin/composite_exams.php', 'Composite exam deleted');
                break;
        }
    } catch (PDOException $e) {
        error_log("Composite exam action error: " . $e->getMessage());
        redirect(APP_URL . '/admin/composite_exams.php', 'Error: ' . $e->getMessage(), 'error');
    }
}
...
// HANDLE UPDATE EXAM
// =====================================================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_composite_exam') {
    $examId = intval($_POST['exam_id']);
    ...
    $shuffle_sub_questions = isset($_POST['shuffle_sub_questions']) ? 1 : 0;
    $show_result = isset($_POST['show_result']) ? 1 : 0;
    
    try {
        $stmt = $db->prepare("
            UPDATE composite_exams SET 
                exam_code = ?,
                exam_title = ?,
                department_id = ?,
                level_id = ?,
        ...
        $stmt->execute([
            ...
            $exam_date, $start_time, $duration, $question_limit, $pass_mark,
            $instruction, $shuffle_questions, $shuffle_sub_questions, $show_result,
            $examId
        ]);
        
        logActivity('admin', $_SESSION['admin_id'], 'update_composite_exam', "Updated composite exam ID: $examId");
        redirect(APP_URL . '/admin/composite_exams.php', 'Composite exam updated successfully');
    } catch (PDOException $e) {
        error_log("Update composite exam error: " . $e->getMessage());
        redirect(APP_URL . '/admin/composite_exams.php', 'Error: ' . $e->getMessage(), 'error');
    }
}
...
if (!empty($_GET['department'])) { 
    $filters[] = "ce.department_id = ?"; 
    $params[] = $_GET['department']; 
}
if (!empty($_GET['search'])) { 
    $filters[] = "(ce.exam_code LIKE ? OR ce.exam_title LIKE ?)"; 
    $p = "%{$_GET['search']}%"; 
...
$examsStmt = $db->prepare("SELECT ce.*, d.dept_name, l.level_name, sem.semester_name, ses.session_name,
    (SELECT COUNT(*) FROM composite_questions WHERE composite_exam_id = ce.id AND status = 1) as question_count,
    (SELECT COUNT(*) FROM composite_sub_questions csq 
        JOIN composite_questions cq ON csq.composite_question_id = cq.id 
        WHERE cq.composite_exam_id = ce.id) as sub_question_count,
    (SELECT COUNT(*) FROM composite_exam_students WHERE composite_exam_id = ce.id) as assigned_students
    ...
    LEFT JOIN sessions ses ON ce.session_id = ses.id
    $where 
    ORDER BY ce.created_at DESC");
$examsStmt->execute($params);
$examsList = $examsStmt->fetchAll();
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="mb-0"><i class="bi bi-diagram-3 me-2" style="color: #9c27b0;"></i>Composite Exams</h4>
        <small class="text-muted">Exams with nested sub-questions (Parent question with A, B, C, D sub-questions)</small>
    </div>
    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#createCompositeExamModal">
        <i class="bi bi-plus-lg me-1"></i>Create Composite Exam
    </button>
</div>
...
<div class="row g-4">
    ...
    <div class="col-lg-6 col-xl-4">
        <div class="card border-0 shadow-sm h-100 composite-exam-card">
            <div class="card-header bg-white d-flex justify-content-between align-items-center">
                <span class="composite-badge">
                    <i class="bi bi-diagram-3 me-1"></i><?php echo e($exam['exam_code']); ?>
                </span>
            ...
            <div class="card-body">
                ...
                <p class="text-muted mb-2">
                    ...
                    <i class="bi bi-diagram-3 me-1"></i><?php echo $exam['question_count']; ?> main Q | 
                    <i class="bi bi-list-nested me-1"></i><?php echo $exam['sub_question_count']; ?> sub-Q | 
                    <i class="bi bi-people me-1"></i><?php echo $exam['assigned_students']; ?> assigned
                </p>
                
                <?php if ($exam['shuffle_questions']): ?>
                    <span class="badge bg-secondary"><i class="bi bi-shuffle me-1"></i>Shuffle Main</span>
                <?php endif; ?>
                <?php if ($exam['shuffle_sub_questions']): ?>
                    <span class="badge bg-secondary"><i class="bi bi-shuffle me-1"></i>Shuffle Sub-Q</span>
                <?php endif; ?>
            </div>
            <div class="card-footer bg-white border-top-0">
                <div class="btn-group w-100">
                    <a href="composite_questions.php?exam_id=<?php echo $exam['id']; ?>" class="btn btn-sm btn-outline-primary" title="Questions">
                        <i class="bi bi-question-circle"></i>
                    </a>
                    <a href="assign_composite_exam.php?exam_id=<?php echo $exam['id']; ?>" class="btn btn-sm btn-outline-success" title="Assign Students">
                        <i class="bi bi-person-check"></i>
                    </a>
                    <a href="monitoring.php?exam_id=<?php echo $exam['id']; ?>&type=composite" class="btn btn-sm btn-outline-info" title="Monitor">
                        <i class="bi bi-tv"></i>
                    </a>
                    <button class="btn btn-sm btn-outline-warning" onclick="editCompositeExam(<?php echo $exam['id']; ?>)" title="Edit">
                    ...
                    <?php if ($exam['status'] === 'draft'): ?>
                        <form method="POST" class="d-inline" onsubmit="return confirm('Publish this composite exam?')">
                    ...
                    <?php elseif ($exam['status'] === 'published'): ?>
                        <form method="POST" class="d-inline" onsubmit="return confirm('Start this composite exam now?')">
                    ...
                    <?php elseif ($exam['status'] === 'running'): ?>
                        <form method="POST" class="d-inline" onsubmit="return confirm('End this composite exam for all students?')">
                    ...
                    <?php elseif ($exam['status'] === 'ended'): ?>
                        <form method="POST" class="d-inline" onsubmit="return confirm('Archive this composite exam?')">
                    ...
                    <?php elseif ($exam['status'] === 'archived'): ?>
                        <form method="POST" class="d-inline" onsubmit="return confirm('Republish this composite exam? It will become available for students again.')">
    ...
    <div class="col-12">
        <div class="text-center py-5 text-muted">
            ...
            <p class="mt-2">No composite exams found. Create your first composite exam!</p>
        </div>
    </div>
    <?php endif; ?>
</div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-arrow-repeat me-2 text-warning"></i>Reset Composite Exam</h5>
...
<!-- ===================================================== -->
<!-- CREATE COMPOSITE EXAM MODAL -->
<!-- ===================================================== -->
<div class="modal fade" id="createCompositeExamModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-plus-circle me-2" style="color: #9c27b0;"></i>Create Composite Exam</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form id="createCompositeExamForm">
                <?php echo csrfField(); ?>
                <div class="modal-body">
                    <div class="alert alert-info small">
                        <i class="bi bi-info-circle me-1"></i>
                        <strong>Composite exams</strong> have questions with nested sub-questions. Each main question contains multiple sub-questions (A, B, C, D) each with their own options.
                    </div>
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">Exam Code *</label>
                            <input type="text" name="exam_code" class="form-control" value="CMP<?php echo date('Y') . strtoupper(bin2hex(random_bytes(3))); ?>" required>
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="question_limit" class="form-control" value="10" min="1" max="100">
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-4">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="shuffle_questions" value="1" checked>
                                <label class="form-check-label">Shuffle Main Questions</label>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="shuffle_sub_questions" value="1" checked>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg me-1"></i>Create Composite Exam</button>
                </div>
            </form>
        </div>
    </div>
...
<!-- ===================================================== -->
<!-- EDIT COMPOSITE EXAM MODAL -->
<!-- ===================================================== -->
<div class="modal fade" id="editCompositeExamModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-pencil-square me-2" style="color: #9c27b0;"></i>Edit Composite Exam</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form id="editCompositeExamForm" method="POST">
                <?php echo csrfField(); ?>
                <input type="hidden" name="action" value="update_composite_exam">
...
async function searchStudentsForReset() {
    ...
    try {
        const url = APP_URL + '/ajax/admin/search_composite_exam_students.php?exam_id=' + examId + 
        ...
        if (data.success && data.students && data.students.length > 0) {
            ...
            resultsContainer.innerHTML = `
                <div class="alert alert-warning small mb-0">
                    ...
                    assigned to this composite exam
                </div>
            `;
        }
    } catch (error) {
...
// =====================================================
// CREATE COMPOSITE EXAM
// =====================================================
document.getElementById('createCompositeExamForm').addEventListener('submit', async function(e) {
    e.preventDefault();
    ...
    try {
        const response = await fetch(APP_URL + '/ajax/admin/create_composite_exam.php', {
            method: 'POST',
            headers: { 'X-CSRF-TOKEN': CSRF_TOKEN },
            body: formData
        });
        ...
        if (data.success) {
            ...
            bootstrap.Modal.getInstance(document.getElementById('createCompositeExamModal')).hide();
...
// EDIT COMPOSITE EXAM
// =====================================================
function editCompositeExam(id) {
    const modalEl = document.getElementById('editCompositeExamModal');
    ...
    fetch(APP_URL + '/ajax/admin/get_composite_exam.php?id=' + id, {
        headers: { 'X-CSRF-TOKEN': CSRF_TOKEN }
    })
